...
class Player
  def hand
    ...
    while true
          if input_hand == "0" || input_hand == "1" || input_hand == "2" # if 「input_hand」が「0, 1, 2」のいずれかの場合だった場合 # ヒント：include?メソッドについて調べてみましょう。
           return input_hand.to_i # 「input_hand」をそのまま返す。# ヒント：戻り値を返して繰り返し処理を終了させたい場合、「return」を使用します。
          else # else それ以外の場合
           puts "0〜2の数字を入力してください。"# プレイヤーに「0〜2」を入力させる文章を表示させる。
           puts "0:グー, 1:チョキ, 2:パー"
           input_hand = gets.chomp # 変数「input_hand」にプレイヤーの入力値を代入します。
          end# end if文のend
    end
  end
end
...
class Janken
  def pon(player_hand, enemy_hand)
    ...
    if player_hand == enemy_hand# Playerクラスの戻り値(player_hand)とEnemyクラスの戻り値(enemy_hand)の値が同じだった場合
       # 「あいこ」を出力します。
       puts "あいこ"
       true#「true」を返してじゃんけんを繰り返し実行させます。
...
#F: Playerクラスの新しいインスタンスを生成し、変数playerに代入している

#② 大文字のPlayerと小文字のplayerの違い
#Playerはクラス名で、playerはPlayerクラスのインスタンスを代入した変数である